...
```{r}
df%>% ggplot (aes(x = SalePrice, y = OverallQual, fill=factor(Neighborhood,ordered=FALSE)))+
...
y =6.8, 10.7, 7.36, 8.02, 8.1
```{r}
sim1a_lm <- lm(formula =y ~x, data = sim1a)
tidy(sim1a_lm)
```

intercept = 5.5, 6.6, 6.3, 5.3, 10.054
```{r}
glance(sim1a_lm)
```
R square = 0.92, 0.85, 0.833, 0.81, 0.1675

After several attemps of running the model, it shows that the values of coefficient and R square keeps changing each time 
